...
void HandGesture::CenterOfHand(Rect oRect, MyImage *m)
{
	Point coor;
	coor.x = oRect.x + oRect.width / 2;
	coor.y = oRect.y + oRect.height / 2;
	circle(m->src, mouse::get_mean(coor, 3), 5, Scalar(0, 0, 255), 4);
	//SetCursorPos(coor.x, 500);
	//mouse::pos(mouse::get_mean(coor));
	//mouse::key(mouse::get_mean(coor, 2), m);
	/*if (fingerTips.size() == 2 && fingerMemb.size() == 1) {
		line(m->src, fingerMemb[0], fingerTips[0], Scalar(255, 0, 255), 2, 12, 0);
		line(m->src, fingerMemb[0], fingerTips[1], Scalar(255, 0, 255), 2, 12, 0);
	}*/
	for (int i = 0; i < fingerMemb.size(); i++) {
		circle(m->src, fingerMemb[i], 5, Scalar(255, 255, 0), 4, 8, 0);
	}
}
void HandGesture::drawFingerTips(MyImage *m){
	...
	/*if (fingerTips.size() == 2 && fingerMemb.size() == 1)
	{
		mouse::get_angle(fingerTips[0], fingerTips[1], fingerMemb[0]);
	}*/
	/*if (fingerTips.size() == 2)
	{
		String inf;
		stringstream ss;
		line(m->src, fingerTips[0], fingerTips[1], Scalar(0, 255, 0), 2, 16, 0);
		ss << mouse::get_dist(fingerTips[0], fingerTips[1]);
		ss >> inf;
		//cout << inf <<endl;
		//putText(m->src, inf, Point(20, 20), fontFace, 1.2f, Scalar(255, 0, 0), 2);
		//cout << mouse::get_dist(fingerTips[0], fingerTips[1]) << endl;
		//cout << "Coor1:" << fingerTips[0].x << "x" << fingerTips[0].y << "Coor2:" << fingerTips[1].x << "x" << fingerTips[1].y << endl;
		if (fingerTips.size() == 5)
		{
			keybd_event('C', 0, 0, 0);
			keybd_event('C', 0, KEYEVENTF_KEYUP, 0);
		}
		//mean.x = mean.x / fingerTips.size();
		//mean.y = mean.y / fingerTips.size();
		//mouse::pos(mean);
		//CenterOfHand(bRect, m);
		//cout << "Mean X=" << _p.x << ", mean Y=" << _p.y << endl;
		//circle( m->src, mean, 5, Scalar(0,0,255), 4 );
	}*/
	/*if (fingerTips.size() == 5)
	{
		keybd_event('W', 0, 0, 0);
